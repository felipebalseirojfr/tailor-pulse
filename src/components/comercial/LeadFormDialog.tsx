...
function buildInitialForm(lead?: Lead | null, defaultResponsavelId?: string) {
  if (lead) {
    return {
...
function LeadFormInner({ onClose, lead }: Omit<Props, "open">) {
  const { data: profiles = [] } = useProfiles();
  const upsert = useUpsertLead();

  const [form, setForm] = useState(() => buildInitialForm(lead, profiles[0]?.id));
  ...
  }, [form, lead, isFinal, upsert, onClose]);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        {open && <LeadFormInner onClose={onClose} lead={lead} />}
      </DialogContent>
    </Dialog>
  );
}
