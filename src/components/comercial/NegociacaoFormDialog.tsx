...
function buildInitialForm(negociacao?: Negociacao | null, defaultResponsavelId?: string) {
  if (negociacao) {
    return {
...
/** Inner form rendered only when dialog is open — avoids useEffect loops */
function NegociacaoFormInner({ onClose, negociacao }: Omit<Props, "open">) {
  const { data: profiles = [] } = useProfiles();
  const upsert = useUpsertNegociacao();

  const [form, setForm] = useState(() => buildInitialForm(negociacao, profiles[0]?.id));
  ...
  }, [form, negociacao, isFinal, upsert, onClose]);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        {open && <NegociacaoFormInner onClose={onClose} negociacao={negociacao} />}
      </DialogContent>
    </Dialog>
  );
}
