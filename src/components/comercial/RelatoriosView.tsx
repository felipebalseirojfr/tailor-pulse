...
export default function RelatoriosView() {
  ...
  const kpis = useMemo(() => {
    ...
    const taxa = fechados + perdidos > 0 ? Math.round((fechados / (fechados + perdidos)) * 100) : 0;
    return { total: filtered.length, fechados, perdidos, taxa };
  }, [filtered]);

  // Charts data (lazy computed)
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            <CardContent className="p-4 text-center">
              <p className="text-3xl font-bold text-success">{kpis.fechados}</p>
              <p className="text-xs text-muted-foreground">Fechados</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
              <p className="text-3xl font-bold text-destructive">{kpis.perdidos}</p>
              <p className="text-xs text-muted-foreground">Perdidos</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
              <p className="text-3xl font-bold text-primary">{kpis.taxa}%</p>
      ...
      {!isLoading && overdueByResp.length > 0 && (
        <Card>
          <CardHeader className="pb-2">
