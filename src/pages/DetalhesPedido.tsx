...
export default function DetalhesPedido() {
  ...
  const handleUndo = async (logId: string, dadosAntes: any) => {
    ...
    try {
      ...
      const { data: profileData } = await supabase.from("profiles").select("nome").eq("id", user?.id).maybeSingle();
      const usuarioNome = profileData?.nome || user?.email || "Usuário desconhecido";

      if (dadosAntes.pedido) {
        const pedidoUpdate: any = {};
        const camposPermitidos = ['quantidade_total', 'observacoes_pedido', 'preco_venda', 'composicao_tecido', 'tecido', 'aviamentos', 'produto_modelo', 'tipo_peca', 'data_inicio', 'prazo_final', 'grade_tamanhos', 'tipos_personalizacao', 'prioridade'];
        for (const campo of camposPermitidos) {
          if (dadosAntes.pedido[campo] !== undefined) pedidoUpdate[campo] = dadosAntes.pedido[campo];
        }
        if (Object.keys(pedidoUpdate).length > 0) {
          const { error } = await supabase.from("pedidos").update(pedidoUpdate).eq("id", id);
        ...
        for (const etapaAntes of dadosAntes.etapas) {
          if (etapaAntes.id) {
            await supabase.from("etapas_producao").update({
              ...
              terceiro_id: etapaAntes.terceiro_id,
            }).eq("id", etapaAntes.id);
          }
        }
      }
      ...
      await (supabase.from("pedidos_auditoria") as any).insert({
        ...
        dados_antes: { pedido: estadoAtualPedido, etapas: estadoAtualEtapas },
        dados_depois: dadosAntes,
      });

      toast({ title: "Alteração desfeita", description: "O pedido foi restaurado para o estado anterior." });
      fetchPedidoDetails();
    } catch (error: any) {
  ...
  useEffect(() => {
    ...
    const etapasChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'etapas_producao', filter: `pedido_id=eq.${id}` }, () => fetchPedidoDetails())
      .subscribe();

    const pedidosChannel = supabase
      .channel(`detalhes-pedido-${id}`)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'pedidos', filter: `id=eq.${id}` }, () => fetchPedidoDetails())
      .subscribe();
  ...
  const fetchPedidoDetails = async () => {
    try {
      ...
      const { data: etapasData, error: etapasError } = await supabase
        ...
        .select("*, profiles(nome), terceiros(nome)")
        .eq("pedido_id", id)
        .order("ordem");
  ...
  const getTerceirosDaEtapa = (tipoEtapa: string) => {
    return terceiros.filter((t) => t.tipo_etapa === tipoEtapa);
  };

  const downloadArquivo = async (caminho: string, nome: string) => {
    try {
      const { data, error } = await supabase.storage.from("pedidos-arquivos").download(caminho);
      if (error) throw error;
      ...
      toast({ title: "Download iniciado", description: `Arquivo ${nome} está sendo baixado.` });
    } catch (error) {
  ...
  const avancarEtapa = async (etapaId: string, dataInicio: Date, dataTerminoPrevista: Date) => {
    try {
      ...
      toast({ title: "Etapa avançada", description: "Etapa concluída e próxima etapa liberada automaticamente." });
      fetchPedidoDetails();
    } catch (error: any) {
      toast({ title: "Erro ao avançar", description: error.message || "Erro desconhecido ao avançar etapa", variant: "destructive" });
    } finally {
  ...
  const handleDeletePedido = async () => {
    if (!pedido) return;
    try {
      const { error: etapasError } = await supabase.from("etapas_producao").delete().eq("pedido_id", pedido.id);
      if (etapasError) throw etapasError;
      const { error: pedidoError } = await supabase.from("pedidos").delete().eq("id", pedido.id);
      if (pedidoError) throw pedidoError;
      toast({ title: "Pedido excluído!", description: "O pedido foi removido do controle de produção." });
      navigate("/pedidos");
    } catch (error: any) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "em_andamento": return <PlayCircle className="h-5 w-5 text-warning" />;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {podeEditar && (
              <Button variant="outline" size="sm" onClick={() => navigate(`/pedidos/${id}/editar`)}>
      ...
      {pedido.arquivos && pedido.arquivos.length > 0 && (
        <Card>
          <CardHeader><CardTitle>Arquivos Anexados</CardTitle></CardHeader>
          <CardContent>
            <div className="space-y-3">
              {pedido.arquivos.map((arquivo, index) => (
                <div key={index} className="flex items-center justify-between rounded-lg border border-border p-4 hover:bg-accent/50 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-muted">{getFileIcon(arquivo.tipo)}</div>
                    <div>
                      <p className="font-medium">{arquivo.nome}</p>
                      <p className="text-sm text-muted-foreground">{formatFileSize(arquivo.tamanho)}</p>
                    </div>
                  </div>
                  <Button variant="outline" size="sm" onClick={() => downloadArquivo(arquivo.caminho, arquivo.nome)}>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          {podeEditar && (
            <Button variant="outline" size="sm" onClick={() => navigate(`/pedidos/${id}/editar`)} className="gap-2">
              <Settings2 className="h-4 w-4" />Editar Etapas
            </Button>
          )}
        </CardHeader>
        <CardContent>
              ...
              return (
                <div key={etapa.id} className="relative">
                  {index !== etapas.length - 1 && (
                    <div className={`absolute left-[10px] top-10 h-full w-0.5 ${etapa.status === "concluido" ? "bg-success" : "bg-muted"}`} />
                  )}
                  <div className="flex gap-4">
                    <div className="relative flex-shrink-0">{getStatusIcon(etapa.status)}</div>
                    <div className="flex-1 space-y-4 rounded-lg border border-border p-4">
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                          ...
                          <Select value={etapa.status} onValueChange={(value) => atualizarEtapa(etapa.id, "status", value)}>
                        ...
                        <div className="space-y-2">
                          <label className="text-sm font-medium">
                            {temTerceiros ? "Terceiro Responsável" : "Responsável Interno"}
                          </label>
                          {temTerceiros ? (
                            <Select
                              value={etapa.terceiro_id || "nenhum"}
                              onValueChange={(value) => atualizarEtapa(etapa.id, "terceiro_id", value === "nenhum" ? null : value)}
                            >
                              <SelectTrigger>
                            ...
                            <Select
                              value={etapa.responsavel_id || "nenhum"}
                              onValueChange={(value) => atualizarEtapa(etapa.id, "responsavel_id", value === "nenhum" ? null : value)}
                            >
                              <SelectTrigger>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeletePedido} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Excluir Pedido
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog open={showChecklist} onOpenChange={setShowChecklist}>
        <AlertDialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <AlertDialogHeader><AlertDialogTitle className="sr-only">Checklist de Produção</AlertDialogTitle></AlertDialogHeader>
      ...
      <AlertDialog open={showFichaCorte} onOpenChange={setShowFichaCorte}>
        <AlertDialogContent className="max-w-[320mm] max-h-[90vh] overflow-auto print:max-w-none print:max-h-none print:overflow-visible">
          <AlertDialogHeader><AlertDialogTitle className="sr-only">Ficha de Corte</AlertDialogTitle></AlertDialogHeader>
          <FichaCorte
            ref={fichaCorteRef}
